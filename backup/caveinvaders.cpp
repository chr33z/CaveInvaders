...
void VRPN_CALLBACK callback_button(void* userData, const vrpn_BUTTONCB button)
{
	if (button.button == 0 && button.state == 1){
		...
		shoot(wand_position, direction, 1500.0, 100, 2000);
	}
...
void animate(){
	...
	if (!player->hasGravity()){
		...
		mgr->setTranslation(player->getPosition());
	}
...
float timeToNextRandomFall = 10000;
float currentTimeToNextRandomFall = 10000;
